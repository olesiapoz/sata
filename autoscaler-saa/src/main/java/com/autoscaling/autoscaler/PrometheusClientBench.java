...
//
//        RequestEntity<Void> request = buildRequest("sum(rate(request_duration_bucket{le=\"1.5\", app=\"demo1\"}["+minutes+"m]))/ignoring(le)sum(rate(request_duration_count{app=\"demo1\"}["+minutes+"m]))*100");
//
...
//        RequestEntity<Void> request = buildRequest("histogram_quantile(0.98, sum(rate(request_duration_bucket{app=\"demo1\"}["+eval_period_seconds+"s])) by (le))*1000");
//
//        ResponseEntity<HashMap<String, Object>> result = restTemplate.exchange(request, responseType);
//        AvgResponse avgResponse = parseAvgResponse(result.getBody());
//
