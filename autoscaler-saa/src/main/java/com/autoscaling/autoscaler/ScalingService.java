...
public class ScalingService {
    ...
    public static final Integer MAX_REPLICAS = 55;
    ...
    public Integer scale(Integer currentReplicasFromAutoscaler) {
        ...
        if(SLANow > (MetricsService.TARGET_SLA*1.00125) && SLABefore-SLANow < 0){            valuesService.setSufficientNumberOfEventsCollectedFlag(true);
        }
    ...
    public Integer calculateUpscalingTargetReplicas(Integer currentReplicas, Velocity velocity, AvgCPU lastKnownAvgCPU, SlaFactor slaFactor) {
        ...
        if ( valuesService.getSLAStatus().equals(SLAStatus.BELOW_TARGET)) {
            ...
            Integer diffTarget = (int) Math.ceil(BigDecimal.valueOf(MetricsService.TARGET_SLA) 
            .subtract((BigDecimal.valueOf(slaFactor.getValue()).multiply(BigDecimal.valueOf(MetricsService.TARGET_SLA))))
            ...
            Integer slaImpactedTarget = slaFactor.getValue() > 0.00000000000001 ? ((int) Math.ceil(BigDecimal.valueOf(desiredReplicas).divide(BigDecimal.valueOf(slaFactor.getValue()),4, RoundingMode.HALF_UP).doubleValue())) + diffTarget : MAX_REPLICAS;
