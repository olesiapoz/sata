 ...
 public class PrometheusClient {
         ...
         txReader.execute(() -> {
             while (true) {
                 try {
                         ...
                         AvgResponse percentileResponse = avgResponse;
                         Throughput throughput = readThroughput();
                         AvgCPU suggestedThreshold = new AvgCPU(DateUtils.now(),valuesService.getCPUSettings().getUpscalingValues().getMid()) ;
     ...
     private AvgResponse readAvgResponseTime() {
         RequestEntity<Void> request = buildRequest("sum(rate(request_duration_sum{app=\"demo1\"}[45s]))/sum(rate(request_duration_count{app=\"demo1\"}[45s]))*1000");
     ...
     private BTCount readMetrics()  {
         RequestEntity<Void> request = buildRequest("sum by (demo1) (rate(bt_count_total{app=\"demo1\"}[45s]))");
         ResponseEntity<HashMap<String, Object>> result = restTemplate.exchange(request, responseType);
